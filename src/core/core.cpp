...
// Generated by build.ps1 into the build directory. Guarded so the source still
// compiles in an editor or IDE that has not run the script -- a build with no
// stamped version says so in the log rather than claiming one it does not have.
#if __has_include("pemf_version.h")
#include "pemf_version.h"
#else
#define PEMF_VER_S "0.0.0-unstamped"
#endif
...
static void RunSafePoint()
{
    ...
    // city table, and it can decide to build a ship.
    {
        ...
        storms::UpdateBattleWeather(content::g_worldLive);
    ...
    // The standing ledger. Primed here rather than off a lifecycle hook so a
    // career that never had a sidecar -- a brand new one, or a save from before
    // this existed -- still gets a baseline the moment it is playable. Cleared
    // on the way out so one career's grudges cannot follow another's.
    if (g_targetOK) {
        if (state::InGame()) {
            if (!standing::g_live) standing::Prime();
            standing::Tick();
        } else if (standing::g_live) {
            standing::Reset("left the career");
        }
    }

    // Hand one queued line to the game's notice strip, if it is free. Must be
    // here rather than in a draw hook: the strip is a single slot shared with
    ...
    // a dialog was never about WHERE the pump ran -- it was the draw hooks being
...
extern "C" void __cdecl PemfOnEndScene(void* /*device*/)
{
    ...
    // NOT OUR FRAME. A modal card runs a NESTED message loop, and that loop
    // renders -- so this hook is re-entered from inside the dialog, on frames
    // the dialog is composing. Drawing there dirties the shared message buffer
    // mid-composite and the card comes up with a half-drawn background.
    //
    // The overworld gate does not catch it. That gate goes stale on a timer
    // (kWorldLiveMaxAgeMs, 250ms) refreshed from the safe point, and the safe
    // point is the main loop -- which the nested loop has displaced. So for the
    // first quarter second of every card it still answers "yes", which is
    // precisely the window the artifact appears in.
    //
    // Pump() already refuses to re-enter itself; this is the other half of that
    // guard, for the draw side.
...
extern "C" void __cdecl PemfOnBeginScene(void* /*device*/)
{
    ...
    // Same reasoning as EndScene, and it matters more here: the world phase
    // dirties the shared buffer BEFORE the sea is drawn, so anything left
    // behind during a card is painted across the water in that same frame.
    if (events::Busy()) return;
...
extern "C" void __cdecl PemfAfterSailingRender(void)
{
    ...
    // A card's nested loop re-enters this path too. See PemfOnEndScene.
    if (events::Busy()) return;
...
static VesselTrack g_track[game::addr::kMaxShips];
...
static void WatchTheWater()
{
    ...
    __try {
        ...
        // What we are SEEN to be is the flag we have run up, which is what the
        // suspicion model tracks. This used to log state::Nationality() -- the
        // ship record's nationality field, which false colours deliberately
        // never writes -- so the line read "we fly Spanish" through an entire
        // playtest spent flying Dutch colours. The value was only ever used in
        // this log, so it misled the reader and nothing else.
        const int worn = suspicion::g_wearing;

        // Same 24-slot window that hid vessels from the suspicion model; the
        // watcher exists to corroborate what that model sees, so it has to look
        // at the same water. Range-gated below, so the extra slots are cheap.
        for (int i = 1; i < game::addr::kMaxShips; ++i) {
            ...
            if (t.have) {
                ...
                // A vessel that is not moving is not chasing anybody. Without
                // the idle case the counter only ever changed while the vessel
                // was under way, so one that stopped stayed "PURSUING" for the
                // rest of the session -- observed in a playtest as a hunter
                // still flagged as chasing at `moved 0` while the distance to
                // her grew from 17,349 to 20,707.
                if (moved > 20 && toward > 0) ++t.pursuing;
                else if (moved > 20)          t.pursuing = 0;   // under way, not at us
                else if (t.pursuing > 0)      --t.pursuing;     // becalmed: cool off
            }
            ...
            if (notable) {
                Log("falsecolours: vessel %2d flies %-7s dist %6d  moved %5d  "
                    ...
                    worn >= 0 ? game::NationName(worn) : "own colours");
            }
        }
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
...
static DWORD WINAPI Init(LPVOID)
{
    ...
    // The version and the build stamp lead the log on purpose. A tester running
    // a stale DLL is indistinguishable from a broken feature otherwise -- the
    // symptoms are identical ("it does nothing"), and dating a build from the
    // strings inside it is not something a bug report should ever require.
    SYSTEMTIME st; GetLocalTime(&st);
    Log("=== PEMF %s loaded === built %s %s | pid=%lu  %04d-%02d-%02d "
        "%02d:%02d:%02d",
        PEMF_VER_S, __DATE__, __TIME__,
