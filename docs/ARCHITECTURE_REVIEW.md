...
| **"Change one thing" means change one thing *that matters* — removing a field is a change too.** | The chase probe withheld the destination write to isolate the target field. But a ship with no fresh destination has ARRIVED, so every pursuer despawned in 2 s and the probe produced no samples at all. Withholding is not neutral. |
| **A control that varies two things controls nothing.** | Run 2 changed the target field *and* spawned 15,000 units away instead of 2,600. The ships died of distance, and the result read as "the target field is fatal". Pin every variable you are not testing, including the ones the world chose for you. |
| **Measure the subject against its own orders, not against yourself.** | "She stays near me" and "she loiters by her home port" are the same numbers when you are parked next to that port. Logging her distance to the port she was *ordered* to settled in one run what two runs of distance-to-player could not. |
| **Two different failures can present as the same nothing.** | "Culled for being too far away" and "docked because she had arrived" both look like *the ship is gone*. They were confused for each other twice before the log line said which. |
| **A magic-number divide is worth checking against the shift.** | `imul 0x66666667` is `/5` with `sar 1` and `/10` with `sar 2`. The comment recorded `sar 1` and the code computed `/10`; the clamp hid it at both ends, so only the middle of the reputation range was wrong. |
| **A fix that removes a symptom can reintroduce it one layer down.** | Aiming the hunter *past* the player cured harbour-parking — until the port nearest the reflected point turned out to be the port she was built in. Same bug, new hat, and the log said `aimed past us at city 24` from a hunter built at city 24. |
| **Grep the docs before naming an address.** | `FUN_004879F0` was "discovered" as an id→text resolver when it was already documented as `WrapText`. Twice now, this range has cost a round to something already written down. |
| **Two assets registered on adjacent lines are not the same asset.** | `cloud00.nif` and `stormcloud_sound.kfm` sit one line apart in the registrar, so the storm patch scaled the ordinary fair-weather cloud for a whole round. The playtest called it exactly right — "the normal clouds look larger" — which is the sort of report that names the bug if you read it literally. |
| **Check the gap between two globals before raising a count that indexes them.** | `cmp eax,3` looked like a taste dial for how much weather to draw. The x and y position arrays are 12 bytes apart — exactly three dwords — so a 4th slot writes one array into the other. A count knob is only a count knob if the storage backs it. |
| **When two visual knobs interact, say so in the tuning file.** | Scale and height fight each other: a big cloud at low altitude swallows the camera and reads as fog, so scale 1000 at height 320 produced a blank grey screen and no visible cloud at all. The ini now says "raise the height whenever you raise the scale" because the number alone teaches nobody. |
| **A same-length instruction rewrite beats hunting for a spare operand.** | Storm scale was `lea edx,[ebx+ebx*4]` + `shl edx,4` — no immediate to tune. Both together are six bytes, exactly `imul edx, ebx, imm32`, so the pair became one tunable instruction with no code displacement and a clean byte-for-byte restore. |
| **Two instructions that appear adjacent in a decompile may not be adjacent in memory.** | The storm's `lea` and `shl` have `push esi` and `lea eax,[esp+0x60]` between them. A six-byte signature over "both" never matched, the site check refused every run, and the patch silently did nothing through several rounds of "looks bigger" — because nobody re-read the log after retargeting. Disassemble the actual byte range, do not infer it from adjacent lines of C. |
| **Re-read the log after changing WHERE you patch, not just whether it builds.** | Same bug. The build was clean, the game ran, the storms looked plausible, and the log said `NOT patching` for four rounds. A verification you do not read is a verification you do not have. |
| **A refusing site check is a success, not a nuisance.** | That same failed match is the only reason six bytes were not written over `push esi`. Design patches so the check CANNOT be satisfied by the wrong bytes, and log loudly when it declines. |
| **Scaling a rendered object without scaling its bound makes it vanish early.** | Storm clouds at 6.9x drew huge but kept a vanilla-sized cull sphere, so the engine dropped them while 70-80% still on screen — reported as the storm teleporting. Any scale patch on a scene-graph object has a matching bound that also needs it. |
| **A knob named for the effect may control the cause, and the cause may not be what you want.** | Raising the weather curve's numerator gives BIGGER raindrops and stronger wind, never DENSER rain. "More rain" and "more of the thing rain is multiplied by" are different asks, and only measurement told them apart. |
| **A frame-delta ramp is not a fade until the delta is clamped.** | A 4-second volume fade completed in 141 ms because one tick had a large `dt`, and `step = 1000 * dt / fadeMs` let it cover the whole range at once. The ramp code was correct and never got to run; it was "fixed" twice by changing the duration before anyone measured the duration. |
| **Instrument the gate, not the number behind it.** | Storm music failed silently three times -- caller passing the wrong flag, then a threshold above the intensity ceiling, then a hysteresis band 18,000 units wide. Each round adjusted a value and hoped. One log line naming what the gate actually saw would have answered all three on the first run. |
| **A readback proves storage, not effect.** | MCI accepted `setaudio volume`, returned success, AND read the value back correctly, while playing at full volume throughout. Verifying that a component remembered your instruction says nothing about whether it obeyed it. |
| **Before fighting the engine for a value, measure how often it writes it.** | The storm position looked like a write-once-per-flag field and is rewritten ~10 times a second. Two designs were built on the wrong assumption; a counter would have cost one run and saved both. |
| **When you cannot reorder a draw, ask whether you can delete it.** | Global rain draws over the clouds with no depth test and engine passes are not reorderable -- but the rain amount is one `imul`, and a constant of zero removes the effect entirely, leaving the cloud's own rain in the right place. |
| **Watch before you interfere.** | Four designs for storm placement were built on beliefs about one value; a 24-second watch-only probe settled it and showed all four were wrong. The probe cost one run. The guesses cost four. |
| **A value changing every frame may be MOVING, not being rewritten.** | "The engine rewrites the storm position ten times a second" and "the storm drifts west at 350 units a second" are the same measurement read two ways, and only the second one is true. The first led to two designs that fought the engine for a value nobody was contesting. |
| **Not calling a function is not the same as it doing nothing.** | Suppressing weather by skipping the engine's draw leaked scene-graph instances every frame -- because that call is where instances are marked reusable -- until clouds stopped appearing anywhere, including prefabs never touched. Draw it at an invisible size instead. |
| **You may not own the value, but you may own the ARGUMENT.** | Writing the storm position fights the engine and always will. Passing a different position to the draw does not, and it was available the whole time. Ask what a hook already has in its hands before fighting for the same thing in memory. |
| **`dt == 0` is not "no time configured".** | A 4-second fade collapsed to one frame because `GetTickCount` granularity gave two calls in the same tick a delta of zero, and the else-branch treated that as "fades off" and snapped to target. Zero elapsed time means DO NOTHING and wait. |
| **A mode that legitimately draws nothing needs a heartbeat.** | Rest periods, gates and thresholds all produce empty skies and silence on purpose, which is indistinguishable from being broken. Every such mode now logs what it is doing and when it will stop. |
